...
class CorrespondenceEstimator:
    ...
    def detect_describe(self, img0):
        ...
        if self.desc_cfg['type']=='none':
            ...
            desc0 = self.descriptor(img0, kps0[:, :2])
...
class EvaluationWrapper:
    ...
    # def extract(self):
    #     split = 'full'
    #     list_images_txt='HPatches_images.txt'
    ...
    #     save_feat_dir = os.path.join(results_dir, "GIFT")
    #     check_directory(results_dir)
    #     check_directory(save_feat_dir)

    #     print('==> Successfully load HPatches image')

    #     iterate = tqdm.tqdm(image_list, total=len(image_list), desc="GIFT HPatches")
    #     feat = {}
    ...
    #         img0 = imread(path)
    #         start1 = time.time()
    #         kps0, desc0 = self.correspondence_estimator.detect_describe(img0)
    #         end1 = time.time()

    #         ## Save the kpts and descriptors
    #         H, W, _ = img0.shape
    #         im_pts = kps0[:1500]
    #         descriptors = desc0[:1500]
