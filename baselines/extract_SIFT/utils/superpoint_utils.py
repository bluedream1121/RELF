...
class SuperPointFrontend(object):
    ...
    def extract_feats(self, img, kps):
        assert img.ndim == 2, 'Image must be grayscale.'
        assert img.dtype == np.float32, 'Image must be float32.'
        assert kps.ndim == 2 and kps.shape[1] == 2, 'Keykpoints must be [pn,2]'
        ...
        inp = torch.from_numpy(inp)
        inp = torch.autograd.Variable(inp).view(1, 1, H, W)
        kps=torch.from_numpy(kps)[None,:,:] # [1,pn,2]
        if self.cuda:
            inp = inp.cuda()
            kps = kps.cuda()
        # Forward pass of network.
        with torch.no_grad():
            ...
            feats=interpolate_feats(inp, kps, coarse_desc)

        feats=feats.cpu().detach().numpy() # [1,pn,f]
        feats=feats[0]
...
class SuperPointWrapper:
    def __init__(self,cfg):
        self.fe=SuperPointFrontend(cfg['weights_path'])
    ...
    def __call__(self,img):
        ...
        if pts.shape[1]==0 or pts is None or desc is None:
            return np.zeros([0,2]), np.zeros([0,256])
        else:
            pts=pts.transpose()[:,:]
            desc=desc.transpose()
            return np.ascontiguousarray(pts, np.float32), np.ascontiguousarray(desc, np.float32)
...
class SuperPointDescriptor:
    ...
    def __call__(self,img,kps):
        ...
        if len(img.shape) > 2:
            img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        img = img.astype(np.float32) / 255.0
        desc = self.fe.extract_feats(img,kps)
        return desc