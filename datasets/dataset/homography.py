...
def extreme_sca_rot(pts, h, w):
    pts=scale_pts(pts, 1.4)
    pts=rotate_pts(pts, 90/180*np.pi)
    return pts
...
def sample_homography(h, w):
    ...
    # fns=[nearest_identity, left_right_move, up_down_move, forward_backward_move, scale_move, rotate_move, extreme_sca_rot]
    # pts2=np.random.choice(fns,p=[0.12,0.2,0.15,0.1,0.2,0.03,0.2])(pts2,h,w) ## update
