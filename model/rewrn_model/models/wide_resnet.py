...
########################################################################################################################
# The following piece of code was adapted from https://github.com/uoguelph-mlrg/Cutout/blob/master/model/wide_resnet.py
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, initial_stride: int = 1, deltaorth: bool = False):
        ...
        self.conv1 = nn.Conv2d(3, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
        self.block1 = NetworkBlock(n, nChannels[0], nChannels[1], block, initial_stride, dropRate)
        # 2nd block
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                if deltaorth:
                    ...
                    o, i, w, h = m.weight.shape
                    if o >= i:
                        m.weight.data.fill_(0.)
                        m.weight.data[:, :, w//2, h//2] = torch.tensor(stats.ortho_group.rvs(max(i, o))[:o, :i])
